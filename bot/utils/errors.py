...
async def with_anthropic_retry(
    ...
    for attempt in range(MAX_RETRIES):
        try:
            ...
            # 529 overloaded или 500
            last_exc = e
            status = getattr(e, "status_code", None)
            if attempt < MAX_RETRIES - 1:
                delay = delays[attempt]
                logger.warning("Claude server error %s, попытка %d/%d, жду %ds", status, attempt + 1, MAX_RETRIES, delay)
                await asyncio.sleep(delay)
            else:
            ...
            last_exc = e
            if attempt < MAX_RETRIES - 1:
                logger.warning("Claude сетевая ошибка, попытка %d/%d, жду 3s", attempt + 1, MAX_RETRIES)
